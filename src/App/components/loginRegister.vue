	...
	<ValidationObserver ref="formValidate" tag="form">
		<div class="login-page">
			<transition name="customslide">
					...
					<div class="content-center">
						...
						<div class="form-group content-center__input">
							<ValidationProvider v-slot="v" name="name" rules="required">
								<input type="text" class="form-control" v-model="login_check" name="name" />
				...
				<!-- -->
				<div class="block-center" v-if="stepe === 'setPassword'" :key="'setPassword'">
					<div class="content-center">
						<a class="content-center__img" href="/">
							<img :src="urlLogo" alt="" />
						</a>
						...
						<div class="form-group content-center__input">
							<ValidationProvider v-slot="v" name="pass" rules="required" ref="refPass">
								<input type="password" class="form-control" v-model="password" name="pass" />
								<div class="text-danger text-small">
									<small v-for="(error, ii) in v.errors" :key="ii" class="d-block">
										{{ error }}
									</small>
						...
						<a class="content-center__forgot-pass" href="/user/password"> Mot de passe oublié ? </a>
						<div class="content-center__btn">
				...
				<div class="block-center" v-if="stepe === 'register'" :key="'register'">
					<div class="content-center">
						...
						>
							<div class="form-group content-center__input">
								<input type="mail" class="form-control" v-model="mail" name="mail" />
							</div>
							<div class="text-danger text-small">
								<small v-for="(error, ii) in v.errors" :key="ii" class="d-block">
									{{ error }}
...
export default {
	...
	methods: {
		...
		async checkUserStatus() {
			...
			if (test)
				utilities.post(url, { name: [{ value: this.login_check }] }).then(resp => {
					this.waiting = "";
		...
		Register() {
			...
			if (this.showPassword) this.$set(this.models, "pass", [{ value: this.password }]);
